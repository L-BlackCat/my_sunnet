...
void MyWorker::operator()(){
    while(true){
        // cout << "do Working..." << worker_id << endl;
        // usleep(1000000);
        shared_ptr<MyService> service = MySunnet::inst->PopGlobalService();
        if(service){
...
void MyWorker::checkAndPopGlobal(shared_ptr<MyService> service){
