...
void SocketWorker::onInit(){
    cout << "SocketWorker onInit()"<<endl;
...
void SocketWorker::onEvent(epoll_event ev){
    cout << "SocketWorker OnEvent()"<<endl;
    int fd = ev.data.fd;
    /**
     * 1.通过fd获取自定义套接字
     ...
     * 3.根据自定义套接字的类型，处理事件
    */
    shared_ptr<Conn> conn = MySunnet::inst->getConn(fd);
    if(!conn){
    ...
    if(conn->type == Conn::MY_CONN_TYPE::listen){
        if(isRead){
            onAccept(conn);
        }
        //  暂时没有监听服务套接字的可写事件
...
void SocketWorker::onAccept(shared_ptr<Conn> conn){
    ...
    //  添加自定义套接字
    MySunnet::inst->AddConn(client_fd,conn->service_id,Conn::MY_CONN_TYPE::client);
    ...
    msg->type = BaseMsg::TYPE::SOCKER_ACCEPT;
...
void SocketWorker::onSocketRW(shared_ptr<Conn> conn,bool isReadable,bool isWritable){
    cout << "SocektWorker onSockerRw fd:"<<conn->fd <<endl;
    //  网络模型只做事件分发，不处理事件
    auto msg = make_shared<SocketRWMsg>();
    msg->type = BaseMsg::SOCKER_RW;
    msg->fd = conn->fd;
